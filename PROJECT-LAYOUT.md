...
- **No `base` in the library `build-depends`** — `base` is implicitly
  available. It's listed in the test suite only because test suites are
  separate compilation units that need explicit deps.
